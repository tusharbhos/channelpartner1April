...
export default function MeetingModal({
  ...
  const formattedDate = new Date(entry.meeting_date + "T00:00:00").toLocaleDateString("en-IN", {
  ...
  const handleWhatsApp = async () => {
    if (customer.phone) {
      window.open(
        `https://wa.me/91${customer.phone}?text=${encodeURIComponent(waMsg)}`,
        "_blank"
      );
    } else {
      try { await navigator.clipboard.writeText(waMsg); } catch { /* ignore */ }
      window.open("https://web.whatsapp.com", "_blank");
      setTimeout(() => {
        alert("📋 Message copied!\n\nWhatsApp Web has opened.\nSearch the contact and paste to send.");
      }, 800);
    }
  };
      ...
      >
        ...
        >
          ...
          <div style={{
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "1rem", color: "#fff", margin: "0 0 4px" }}>
                {customer.nickname}
              </p>
          ...
          <div style={{
            ...
            padding: "1rem 1.2rem", display: "flex", flexDirection: "column", gap: "0.85rem",
          }}>
            {/* Status */}
            ...
            <div style={{
              ...
              <div>
                ...
                <p style={{ fontSize: "0.85rem", fontWeight: 700, color: "var(--navy-900)", margin: 0 }}>{formattedDate}</p>
                {entry.meeting_time && (
                  <p style={{ fontSize: "0.85rem", fontWeight: 700, color: "var(--orange-600)", margin: 0 }}>
                    🕐 {formattedTime}
                  </p>
                )}
              </div>
            ...
            {customer.notes     && (
              <div style={{
                ...
                fontSize: "0.8rem", fontStyle: "italic", color: "var(--color-text-secondary)",
              }}>
                📝 "{customer.notes}"
              </div>
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.6rem" }}>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  ...
                  <span style={{ fontSize: "0.82rem", fontWeight: 700 }}>WhatsApp</span>
                </div>
                <span style={{ fontSize: "0.62rem", opacity: 0.85, textAlign: "center", lineHeight: 1.3 }}>
                  {customer.phone ? customer.phone : "Opens WhatsApp Web"}
                </span>
              </button>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  ...
                  <span style={{ fontSize: "0.82rem", fontWeight: 700 }}>Gmail</span>
                </div>
                <span style={{ fontSize: "0.62rem", opacity: 0.85, textAlign: "center", lineHeight: 1.3 }}>
                  {customer.email ? customer.email : "Opens Gmail compose"}
                </span>
              </button>
            </div>
            ...
            {!customer.phone && !customer.email && (
              <div style={{
                ...
                fontSize: "0.74rem", color: "var(--orange-700)",
                display: "flex", gap: 6, alignItems: "flex-start",
              }}>
                <span style={{ fontSize: "0.85rem", flexShrink: 0 }}>💡</span>
                <span>No phone or email saved. WhatsApp Web will open with message copied. Gmail will open with a blank To: field.</span>
              </div>
            )}
          </div>
