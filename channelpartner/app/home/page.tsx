...
function AddProjectBanner({ onAdd }: { onAdd: () => void }) {
  return (
    ...
    >
      ...
      <button onClick={onAdd} className="btn btn-primary flex-shrink-0 w-full sm:w-auto gap-2" style={{ whiteSpace: "nowrap" }}>
...
function SkeletonCard() {
  return (
    <article className="card flex flex-col" style={{ borderRadius: "var(--radius-xl)", overflow: "hidden", background: "#fff" }}>
      <div className="skeleton" style={{ height: "clamp(120px,22vw,152px)", borderRadius: 0 }} />
...
function ProjectCardUI({ project, onSchedule }: { project: ApiProject; onSchedule: (name: string) => void }) {
  ...
  return (
    <article className="card project-card-glow flex flex-col" style={{ borderRadius: "var(--radius-xl)", overflow: "hidden", background: "#fff" }}>
      {image ? (
        ...
        <div style={{ height: "clamp(120px,22vw,152px)", background: "linear-gradient(135deg,var(--navy-900) 0%,var(--navy-700) 100%)", position: "relative", overflow: "hidden" }}>
      ...
      <div className="p-3.5 md:p-4 flex flex-col flex-1" style={{ gap: "0.6rem" }}>
        <div>
          ...
          <p className="text-xs truncate mt-0.5" style={{ color: "var(--color-text-muted)" }}>{developer}</p>
        </div>
        <p className="text-xs" style={{ color: "var(--color-text-muted)", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
          📍 {location}
        </p>
        <p className="font-bold" style={{ fontFamily: "var(--font-display)", color: "var(--orange-600)", fontSize: "clamp(0.9rem,2.5vw,1.05rem)" }}>
          {toCardPrice(project)}
        </p>
        ...
        <div className="grid grid-cols-2 gap-1.5 flex-1">
          {[
            ...
            <div key={info.key} className="px-2 py-1.5 rounded-lg" style={{ background: "var(--slate-50)", border: "1px solid var(--slate-100)" }}>
              <div className="flex items-center gap-1 mb-0.5" style={{ color: "var(--color-text-hint)" }}>
                <InfoIcon type={info.icon} />
                <p style={{ fontSize: "9px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>{info.label}</p>
              </div>
              <p className="text-xs font-semibold truncate" style={{ color: "var(--color-text-primary)" }}>{info.val}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between gap-2 mt-auto pt-1">
          ...
          <button onClick={() => onSchedule(title)} className="btn btn-gold" style={{ fontSize: "0.75rem", padding: "0.4rem 0.9rem", flexShrink: 0 }}>
            Schedule
          </button>
        </div>
      </div>
...
export default function HomePage() {
  ...
  const [filterOptions, setFilterOptions] = useState<SidebarOptions>({
    projects: [], categories: [], tags: [], amenities: [],
  ...
  useEffect(() => {
    ...
    const loadMeta = async () => {
      try {
        ...
        const developmentStatus = (filtersMap.get("development_status")?.options ?? [])
          .map((o) => ({ label: normalize(o.name), value: normalize(o.value ?? o.name).toLowerCase() }))
        ...
        if (!active) return;
        setFilterOptions((prev) => ({ ...prev, categories, tags, amenities, developers, locations, developmentStatus, bestSuited, unitTypes, areaRange, priceRange }));
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => {
      ...
      return matchSearch && matchProject && matchDeveloper && matchLocation && matchCategories && matchTags && matchAmenities && matchStatus && matchBestSuited && matchUnitType && matchArea && matchPrice && matchUnits && matchPossessionExact && matchPossessionWithinYears;
    });
  ...
  return (
    <div className="bg-main min-h-screen flex flex-col">
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed z-50 animate-fade-in-up" style={{ bottom: "1.25rem", right: "1rem", left: "1rem", display: "flex", justifyContent: "center" }}>
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg max-w-sm w-full" style={{ background: "var(--navy-900)", color: "#fff" }}>
      ...
      <main className="flex-1 overflow-y-auto" style={{ paddingTop: "var(--header-height)" }}>
        ...
        {/* Search Bar Banner */}
        <div className="px-3 sm:px-4 md:px-8 py-3 md:py-4" style={{ background: "var(--gradient-header)" }}>
          <div className="max-w-7xl mx-auto">
            <SearchBar value={search} onChange={setSearch} onFilterClick={() => setSidebarOpen(true)} activeFilterCount={activeFilterCount} />
          </div>
        </div>
        ...
        {filteredProjects.length > 0 && (
          <div className="px-3 sm:px-4 md:px-8 py-2"
            style={{ background: "rgba(255,255,255,0.92)", borderBottom: "1px solid var(--slate-100)" }}>
            <div className="max-w-7xl mx-auto flex items-center justify-between">
              ...
              {activeFilterCount > 0 && (
                <button
                  ...
                  className="text-xs font-bold" style={{ color: "var(--red-600)" }}
                >
                  Clear filters
                </button>
              )}
        ...
        <div className="px-3 sm:px-4 md:px-8 py-4 md:py-6 max-w-7xl mx-auto">
          {filteredProjects.length === 0 ? (
            <div className="text-center py-14">
              ...
              <h3 className="text-lg font-bold mb-1" style={{ color: "var(--color-text-muted)" }}>No projects found</h3>
              <p className="text-sm mb-5" style={{ color: "var(--color-text-hint)" }}>Try adjusting your search or filters</p>
            ...
            <>
              ...
              <div className="grid-auto-fill-280 stagger">
                {visibleProjects.map((project) => (
                  <ProjectCardUI key={project.id} project={project} onSchedule={handleSchedule} />
                ))}

                {/* Skeleton placeholders while loading more */}
                {loadingMore && Array.from({ length: Math.min(PAGE_SIZE, filteredProjects.length - visibleCount) }).map((_, i) => (
              ...
              {/* ── End-of-list message ── */}
              {!hasMore && !loadingMore && filteredProjects.length > PAGE_SIZE && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: "0.78rem", fontWeight: 600, color: "var(--navy-600)" }}>
              ...
              {hasMore && !loadingMore && (
                <div style={{ textAlign: "center", marginTop: "1.5rem" }}>
                  <button
                    ...
                    style={{ fontSize: "0.82rem", padding: "0.55rem 1.5rem", gap: 6 }}
                  >
